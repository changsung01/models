...
class TrivialKerasBenchmark(keras_benchmark.KerasBenchmark):
  ...
  def benchmark_xla_1_gpu_fp16(self):
    """Test trivial Keras model (input pipeline) with XLA, 1 GPU and fp16."""
    self._setup()

    FLAGS.num_gpus = 1
    FLAGS.enable_eager = True
    FLAGS.enable_xla = True
    FLAGS.model_dir = self._get_model_dir('benchmark_xla_1_gpu_fp16_trivial')
    FLAGS.batch_size = 256
    self._run_and_report_benchmark()
  ...
  def benchmark_graph_xla_1_gpu_fp16(self):
    ...
    FLAGS.enable_xla = True
    FLAGS.model_dir = self._get_model_dir(
        'benchmark_graph_xla_1_gpu_fp16_trivial')
    FLAGS.batch_size = 256
    self._run_and_report_benchmark()
